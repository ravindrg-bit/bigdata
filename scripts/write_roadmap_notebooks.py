...
def main() -> None:
    write_notebook(
        ...
        [
            ...
            code_cell([
                "from src.train_gnn import train_graphsage, build_graph_tensors, split_indices, compute_metrics, GraphSAGEClassifier",
                "train_graphsage(",
                ...
                "    embeddings_out=Path('data/gnn_embeddings.parquet'),",
    ...
    write_notebook(
        ...
        [
            ...
            code_cell([
                "features = pd.read_parquet('data/features.parquet')",
                "emb = pd.read_parquet('data/gnn_embeddings.parquet')",
    ...
    write_notebook(
        ...
        [
            ...
            code_cell([
                "from src.fairness_audit import run_fairness_audit",
                "report = run_fairness_audit(",
                "    features_path=Path('data/features.parquet'),",
                "    embeddings_path=Path('data/gnn_embeddings.parquet'),",
                "    model_path=Path('models/hybrid_ensemble.pkl'),",
                "    output_json=Path('data/fairness_report.json'),",
                "    output_csv=Path('data/fairness_summary.csv'),",
                "    seed=42,",
                ")",
                "report['pass_criteria']",
            ]),
            code_cell([
                "summary = pd.read_csv('data/fairness_summary.csv')",
                "summary",
            ]),
            code_cell([
            ...
            code_cell([
                "loaded = json.loads(Path('data/fairness_report.json').read_text(encoding='utf-8'))",
    ...
    write_notebook(
        ...
        [
            ...
            code_cell([
                "from src.federated_training import train_federated",
                "report = train_federated(",
                "    features_path=Path('data/features.parquet'),",
                "    model_out=Path('models/federated_model.pt'),",
                "    report_out=Path('data/federated_report.json'),",
                "    seed=42, rounds=8, local_epochs=2, learning_rate=0.01,",
                ")",
                "report",
            ]),
            code_cell([
                "loaded = json.loads(Path('data/federated_report.json').read_text(encoding='utf-8'))",
                "pd.DataFrame([",
