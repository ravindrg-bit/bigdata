...
from src.cold_start import ColdStartScorer
...
from src.federated_training import FederatedInferenceEngine
...
federated_report_path = Path("data/federated_report.json")
fairness_report_path = Path("data/fairness_report.json")
override_log_path = Path("data/override_log.csv")
model_card_path = Path("docs/model_card.md")

if not features_path.exists():
    st.warning("data/features.parquet not found. Run data generation and feature engineering first.")
    st.code("python src/generate_data.py --seed 42 --output-dir data")
    st.code("python src/feature_engineering.py --data-dir data --output data/features.parquet")
    st.stop()
...
with tab3:
    ...
    if fairness_report_path.exists():
        ...
        st.warning("Fairness report not found. Run src/fairness_audit.py first.")
